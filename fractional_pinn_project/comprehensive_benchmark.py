...
class ComprehensiveBenchmark:
    """
    Comprehensive benchmark for comparing all fractional parameter estimation methods.
    """
    
    def __init__(self, 
                 save_results: bool = True,
                 results_dir: str = "comprehensive_benchmark_results",
                 device: str = 'auto'):
        """
        ...
        self.config = {
            ...
            'n_samples_per_config': 5,  # Reduced for faster execution
            'contamination_types': [
                'none', 'noise', 'outliers', 'trend', 'seasonality'
            ],
            'data_types': ['fbm', 'fgn', 'arfima', 'mrw'],
            'neural_training_epochs': 200,  # Reduced for faster execution
            'neural_early_stopping_patience': 30
        }
        
        logger.info(f"ComprehensiveBenchmark initialized on device: {self.device}")
                        ...
                        for contamination in self.config['contamination_types']:
                            ...
                            for sample_idx in range(self.config['n_samples_per_config']):
                                ...
                                if contamination != 'none':
                                    contaminated_data = self.data_generator.add_contamination(
                                        data['data'], contamination_type=contamination
                                    )
                                    # Update the data dictionary with contaminated data
                                    data['data'] = contaminated_data['data']
                                    data['contamination_info'] = {
                                        'type': contamination,
                                        'intensity': contaminated_data['intensity']
                                    }
    ...
    def benchmark_ml_estimators(self, benchmark_data: Dict[str, Any]) -> pd.DataFrame:
        ...
        # Prepare training data (use first 3 samples from each config for training)
        training_data = []
    ...
    def benchmark_neural_models(self, benchmark_data: Dict[str, Any]) -> pd.DataFrame:
        ...
        try:
            ...
            pinn_history = pinn_estimator.train(
                ...
                save_model=True,
                model_description="PINN for comprehensive benchmark",
        ...
        try:
            ...
            pino_history = pino_trainer.train(
                ...
                early_stopping_patience=self.config['neural_early_stopping_patience'],
                save_model=True,
                model_description="PINO for comprehensive benchmark",
        ...
        try:
            ...
            neural_ode_history = neural_ode_trainer.train(
                ...
                early_stopping_patience=self.config['neural_early_stopping_patience'],
                save_model=True,
                model_description="Neural Fractional ODE for comprehensive benchmark",
        ...
        try:
            ...
            neural_sde_history = neural_sde_trainer.train(
                ...
                early_stopping_patience=self.config['neural_early_stopping_patience'],
                save_model=True,
                model_description="Neural Fractional SDE for comprehensive benchmark",
    ...
    def generate_benchmark_report(self):
        ...
        # Calculate statistics by contamination
        contamination_stats = combined_df.groupby(['model_type', 'estimator', 'contamination']).agg({
            'absolute_error': ['mean', 'std'],
            'relative_error': ['mean', 'std']
        }).round(4)
        ...
        best_by_data_type = combined_df.loc[combined_df.groupby(['estimator', 'data_type'])['absolute_error'].idxmin()]
        ...
        with open(report_filepath, 'w') as f:
            ...
            f.write("PERFORMANCE BY CONTAMINATION\n")
            f.write("-" * 30 + "\n")
            f.write(contamination_stats.to_string())
            f.write("\n\n")
    ...
    def plot_benchmark_results(self):
        ...
        performance_data = combined_df.groupby(['model_type', 'estimator'])['absolute_error'].mean().reset_index()
        ...
        # 5. Performance by contamination
        ax5 = plt.subplot(3, 3, 5)
        contamination_performance = combined_df.groupby(['model_type', 'contamination'])['absolute_error'].mean().unstack()
        contamination_performance.plot(kind='bar', ax=ax5)
        ax5.set_title('Performance by Contamination')
        ax5.set_ylabel('Mean Absolute Error')
        ax5.tick_params(axis='x', rotation=45)
        ax5.legend(title='Contamination', bbox_to_anchor=(1.05, 1), loc='upper left')
        
        # 6. Computation time comparison
        ax6 = plt.subplot(3, 3, 6)
        time_comparison = combined_df.groupby(['model_type', 'estimator'])['computation_time'].mean().reset_index()
        sns.boxplot(data=combined_df, x='model_type', y='computation_time', ax=ax6)
        ax6.set_title('Computation Time by Model Type')
        ...
        for model_type in combined_df['model_type'].unique():
            ...
            ax7.hist(data, alpha=0.7, label=model_type, bins=20)
        ax7.set_title('Error Distribution by Model Type')
        ax7.set_xlabel('Absolute Error')
        ax7.set_ylabel('Frequency')
        ax7.legend()
        
        # 8. True vs Estimated scatter plot
        ax8 = plt.subplot(3, 3, 8)
        for model_type in combined_df['model_type'].unique():
            data = combined_df[combined_df['model_type'] == model_type]
            ax8.scatter(data['true_hurst'], data['estimated_hurst'], alpha=0.6, label=model_type)
        ax8.plot([0, 1], [0, 1], 'k--', alpha=0.5)
        ax8.set_title('True vs Estimated Hurst Values')
        ax8.set_xlabel('True Hurst')
        ax8.set_ylabel('Estimated Hurst')
        ax8.legend()
        
        # 9. Performance heatmap
        ax9 = plt.subplot(3, 3, 9)
        heatmap_data = combined_df.groupby(['model_type', 'data_type'])['absolute_error'].mean().unstack()
        sns.heatmap(heatmap_data, annot=True, fmt='.3f', cmap='YlOrRd', ax=ax9)
        ...
        # Save plot
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        plot_filename = f"comprehensive_benchmark_results_{timestamp}.png"
...
def main():
    ...
    print("Comprehensive Benchmark for Fractional Parameter Estimation Methods")
    print("=" * 70)
