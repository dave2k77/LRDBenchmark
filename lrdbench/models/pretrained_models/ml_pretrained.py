...
class RandomForestPretrainedModel(BasePretrainedModel):
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        ...
        for i in range(data.shape[0]):
            ...
            # Remove trend
            detrended = series - np.polyval(np.polyfit(np.arange(len(series)), series, 1), np.arange(len(series)))
            ...
            if n > 10:
                ...
                segments = [detrended[j:j+segment_size] for j in range(0, n, segment_size) if j + segment_size <= n]
        ...
        confidence_interval = (max(0.1, mean_hurst - 1.96 * std_error), 
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        ...
        for i in range(data.shape[0]):
            ...
            if len(series) > 10:
                ...
                segments = [series[j:j+segment_size] for j in range(0, len(series), segment_size) if j + segment_size <= len(series)]
