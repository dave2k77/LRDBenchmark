...
class PerformanceMonitor:
    ...
    def start_monitoring(self, estimator_name: str, data_length: int, parameters: Dict[str, str]) -> str:
        """
    ...
    def get_performance_summary(self, days: int = 30) -> PerformanceSummary:
        ...
        memory_efficiency = avg_memory_usage / avg_execution_time if avg_execution_time > 0 else 0
    ...
    def _identify_bottlenecks(self, metrics: List[PerformanceMetrics]) -> List[str]:
        ...
        bottlenecks = [
            name for name, avg_time in estimator_avg_times.items()
            if avg_time > threshold
        ]
        
        return sorted(bottlenecks, key=lambda x: estimator_avg_times[x], reverse=True)
    
    def get_estimator_performance(self, estimator_name: str, days: int = 30) -> Dict[str, float]:
        ...
        with self._lock:
            estimator_metrics = [
                m for m in self._metrics
                if m.estimator_name == estimator_name and 
