"""
Enhanced GRU Estimator for Long-Range Dependence Analysis

This module provides an enhanced GRU-based estimator with adaptive input sizes,
...
try:
    ...
    warnings.warn("PyTorch not available. Enhanced GRU estimator will not work.")
...
class AdaptiveGRU(nn.Module):
    """
    Enhanced GRU model with adaptive architecture.
...
class EnhancedGRUEstimator(BaseMLEstimator):
    """
    Enhanced GRU estimator for Hurst parameter estimation.
    ...
    def __init__(self, **kwargs):
        """
        Initialize the enhanced GRU estimator.
        ...
        **kwargs : dict
            ...
            - num_layers: int, number of GRU layers (default: 3)
            ...
            - model_save_path: str, path to save trained models (default: 'models/enhanced_gru')
        """
    ...
    def _create_model(self, input_size: int = 1) -> AdaptiveGRU:
        """
    ...
    def _prepare_data(self, data: np.ndarray) -> torch.Tensor:
        """
        Prepare data for GRU input with enhanced preprocessing.
    ...
    def _create_training_data(self, data_list: List[np.ndarray], labels: List[float]) -> Tuple[DataLoader, DataLoader]:
        ...
        for data, label in zip(data_list, labels):
            ...
            # Reshape for GRU: (seq_len, features)
    ...
    def train_model(self, data_list: List[np.ndarray], labels: List[float], save_model: bool = True) -> Dict[str, Any]:
        ...
        print(f"Training Enhanced GRU with input size: {input_size}")
    ...
    def _save_model(self):
        ...
        model_path = os.path.join(save_path, "enhanced_gru_model.pth")
        torch.save({
    ...
    def _try_load_pretrained_model(self) -> bool:
        """
        Try to load a pretrained PyTorch model for enhanced GRU.
    ...
    def estimate(self, data: np.ndarray) -> Dict[str, Any]:
        """
        Estimate Hurst parameter using enhanced GRU.
        ...
        if self._try_load_pretrained_model():
            ...
            method = "Enhanced GRU (Untrained Neural Network)"
    ...
    def get_model_info(self) -> Dict[str, Any]:
        """
        Get information about the enhanced GRU model.
        ...
        info = {
            "model_type": "EnhancedGRU",
            "architecture": "Enhanced GRU with Attention and Bidirectional Layers",
            "hidden_size": self.parameters["hidden_size"],
            "num_layers": self.parameters["num_layers"],
            "dropout_rate": self.parameters["dropout_rate"],
            "bidirectional": self.parameters["bidirectional"],
